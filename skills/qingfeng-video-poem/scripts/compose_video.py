...
def parse_args():
    ...
    p.add_argument("--size", default="720x1280", help="输出分辨率，如 720x1280 / 1280x720 / 1080x1920")
...
def build_clip(src, dst, seg, clip_dur, size, fps, font_rel, sub_file_rel,
    ...
    if title_cfg:
        ...
        vf.append(
            ...
            f"fontcolor=white:fontsize={int(font_size * 1.45)}:"
...
def main():
    ...
    font_size = args.font_size or max(34, int(int(w) * 0.075))
    ...
    if args.title_text:
        ...
        tfile = "_title.txt"
        with open(os.path.join(workdir, tfile), "w", encoding="utf-8") as f:
            f.write(args.title_text)
        title_cfg = {"file": tfile, "start": t_start, "end": t_end}

    print(f"镜头数: {len(scenes)}  分辨率: {args.size}  字号: {font_size}  转场: {transition}s")
    print(f"字体: {font_src}")
    ...
    for i, (src, seg) in enumerate(zip(scenes, segs)):
        ...
        if not args.no_subtitle and seg["text"]:
            ...
            with open(os.path.join(workdir, sub_rel), "w", encoding="utf-8") as f:
                f.write(seg["text"])
        ...
        build_clip(src, dst, seg, clip_dur, args.size, args.fps,
    ...
    print("拼接转场...")
    silent = "_silent.mp4"
    concat_clips(clips, silent, targets, transition, args.size, args.fps, workdir)
