# -*- coding: utf-8 -*-
"""
toutiao_publish_v26.py - 头条号文章发布脚本 v26
改进点:
1. 文字和图片按HTML原始顺序交替插入（不再分两步）
2. 每次图片上传后按ESC退出图片模式
3. 图片目录 = HTML文件所在目录（不是images子目录）
4. 无emoji（避免Windows GBK编码错误）
"""
import websocket, json, time, urllib.request, base64, os, re, sys

CDP_PORT = 9222
HTML_FILE = r"D:\办公\宿松县教育局\PPT\遇见AI\AIyj\3\index.html"
...
def main():
    ...
    print(f"Blocks: {len(blocks)} (text/title={text_cnt}, images={img_cnt})")
    ...
    for i, block in enumerate(blocks):
        ...
        if btype in ('text', 'title'):
            # === 文本块 ===
            if btype == 'title':
                level = block[1]   # int: 2 or 3
                text = block[2]    # str: actual text
            else:
                level = None
                text = block[1]    # str: paragraph text
            ...
            # (b) 按 ESC 确保无残留 dialog/模式
            press_esc(ws)
            time.sleep(0.3)

            # (c) 点击图片按钮
            mouse_click(ws, btn_x, btn_y)
            time.sleep(2.5)  # 等待素材对话框弹出
    ...
    spath = take_screenshot(ws, f"toutiao_v26_final_{ts}.png")
