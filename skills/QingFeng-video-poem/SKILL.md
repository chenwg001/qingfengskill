...
把一首诗（只需给诗名，或给全文）自动变成一段画面精致、剧情连贯、音画同步的短视频。**默认竖屏 9:16**；当用户明确要求横屏（如「横屏 / 16:9 / 横版 / landscape」）时，按 16:9 横屏成片。
...
> **⚠️ 画面方向（orientation）**：**默认竖屏 9:16（portrait）**，无需用户额外说明。只有当用户指令明确出现横屏意图关键词——「横屏 / 横版 / 宽屏 / 16:9 / 横屏视频 / landscape / 横向」等——才切到横屏 16:9（landscape）。判定结果写入 `script.json` 的 `orientation` 字段，贯穿阶段 1–5：
> - `portrait`（默认）：首帧 `1024x1536`，成片 `720x1280`
> - `landscape`：首帧 `1536x1024`，成片 `1280x720`
> 阶段 0.5 会就此再向用户确认一次，避免误判。
...
- **画面方向（第三个问题）**：问用户要竖屏还是横屏。
  - 选项：「竖屏 9:16（默认·推荐）」「横屏 16:9」。
  - **默认把阶段 0 判定的 `orientation` 作为推荐项置顶**（用户指令含横屏关键词时横屏置顶，否则竖屏置顶），让用户一键确认或纠偏。
  - 用户选定后，**回写 `<workdir>/script.json` 的 `orientation` 字段**（与阶段 0 判定对齐），阶段 1/2/5 直接读它。
...
- `size`: **读 `script.json.orientation`** 决定——`portrait`（默认）用 `"1024x1536"`，`landscape` 用 `"1536x1024"`（横屏成片的关键：首帧是横图，VideoGen 才会推断 16:9）
...
- **不要传 `aspect_ratio`**（当前 VideoGen 后端不支持该参数，会报 400）；画面比例由输入首帧图自动推断——**竖图首帧(1024x1536)→竖屏成片，横图首帧(1536x1024)→横屏成片**，所以首帧尺寸务必与 `script.json.orientation` 一致（见阶段 1）。
...
python <skill>/scripts/compose_video.py \
  ...
  --orientation <script.json 的 orientation> --output <题名>.mp4
```
...
- **画面方向**：传 `--orientation portrait`（默认，成片 720x1280）或 `--orientation landscape`（成片 1280x720）；也可直接传 `--size 720x1280 / 1280x720` 显式覆盖。该值必须与阶段 1 首帧尺寸一致（竖图首帧→portrait，横图首帧→landscape）
