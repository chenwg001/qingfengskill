# -*- coding: utf-8 -*-
"""导航到头条号图文发布页面（简化版 v2）"""
import sys, json, time, subprocess, os, urllib.request, websocket
...
print('检查 CDP 端口 {}...'.format(PORT))
pages = get_pages()

if not pages:
    print('端口未监听，尝试启动 Edge...')
    edge_exe = find_edge_exe()
    if not edge_exe:
        print('ERROR: 找不到 Edge 可执行文件')
        print('请手动启动 Edge: msedge --remote-debugging-port=9222 --remote-allow-origins=*')
        sys.exit(1)
    
    print('使用 Edge: {}'.format(edge_exe))
    subprocess.Popen([
        edge_exe,
...
if 'graphic/publish' in current_url:
    ...
    print('WARNING: URL 可能不正确，请手动检查浏览器')
