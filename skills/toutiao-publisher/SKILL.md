...
启动后通过 CDP 自动导航到编辑器：
1. 请求 `http://127.0.0.1:9222/json` 获取页面列表
2. 找到一个 `type=page` 的标签页
3. 连接其 WebSocket，执行 `Page.navigate` 导航到 `https://mp.toutiao.com/profile_v4/graphic/publish`
4. 等待页面加载完成

> 如果 Chrome 已在运行且 9222 端口已监听，则跳过启动步骤，直接导航。
